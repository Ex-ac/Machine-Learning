...
californiaHousingDataFrame = californiaHousingDataFrame.reindex(
    np.random.permutation(californiaHousingDataFrame.index))
...
linearRegressor = tf.estimator.LinearRegressor(
    feature_columns=featureColumns, optimizer=myOptimizer)
...
def trainModel(learningRate, steps, inputData, inputFeatures=[], inputLabels=[], batchSize=10):
    ...
    featureColumns = [tf.feature_column.numeric_column(
        item) for item in inputFeatures]

    def trainInputFun(): return myInputFn(
        featureData, labelData, batchSize=batchSize)

    def predictionInputFun(): return myInputFn(
        featureData, labelData, numEpochs=1, shuffle=False)

    optimizer = tf.train.GradientDescentOptimizer(learning_rate=learningRate)
    optimizer = tf.contrib.estimator.clip_gradients_by_norm(optimizer, 5.0)

    linearRegressor = tf.estimator.LinearRegressor(
        feature_columns=featureColumns, optimizer=optimizer)
    ...
    for i in range(0, period):
        ...
        rootMeanSquaredError = math.sqrt(
            metrics.mean_squared_error(predictions, labelData))

        weights = [linearRegressor.get_variable_value(
            "linear/linear_model/%s/weights" % feature)[0] for feature in inputFeatures]
...
trainModel(0.00001, 2000, californiaHousingDataFrame, [
           "population"], ["median_house_value"], 200)
